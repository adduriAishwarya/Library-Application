...
public class LibrarianController {
	...
	public String updateHoursWorked(
		...
		if (librarian != null) {
			...
			// Write the updated data to the credentials CSV file
			updateCredentialsCSV(librarian);
	...
	private void updateCredentialsCSV(Librarian librarian) {
		...
		// Find and update the entry for the librarian
		for (String[] data : credentialsData) {
			if (data.length >= 4 && "Lib".equals(data[3]) && librarian.getUsername().equals(data[1])) {
				// Update the hoursWorked value in the CSV data
				data[4] = String.valueOf(librarian.getHoursWorked());
				break; // Stop after finding and updating the entry
			}
		}
	
		// Write the updated data back to the credentials CSV file
